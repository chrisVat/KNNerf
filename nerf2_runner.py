...
class NeRF2_Runner():
    ...
    def __init__(self, mode, dataset_type, **kwargs) -> None:
        ...
        # recursively create dir if it doens't exist
        ...
        self.train_iter = DataLoader(train_set, batch_size=self.batch_size, shuffle=False, num_workers=0)
    ...
    def eval_network_spectrum(self):
        ...
        # load all tx poses from tx_pos.csv
        # tx_pos = np.loadtxt(os.path.join(self.datadir, 'tx_pos.csv'), delimiter=',') #
        # this loading doesn't work, the row is the name of the columns, reload it
        tx_pos = np.loadtxt(os.path.join(self.datadir, 'tx_pos.csv'), delimiter=',', skiprows=1)


        # these are x, y, z coordinates


        # load all train indexes from train_index.txt
        train_index = np.loadtxt(os.path.join(self.datadir, 'train_index.txt'), dtype=np.int32)
        print("Train indexes shape: ", train_index.shape)
        train_positions = tx_pos[train_index-1]

        tx_pos_dir = os.path.join(self.datadir, 'tx_pos.csv')
        train_labels = self.train_iter.dataset.nn_labels
        test_labels = self.test_iter.dataset.nn_labels
        train_index_dir = os.path.join(self.datadir, 'train_index.txt')
        test_index_dir = os.path.join(self.datadir, 'test_index.txt')

        knn = KNN(k=1, pos_path=tx_pos_dir, index_path=train_index_dir, labels=train_labels)

        # self.test_iter = self.train_iter # just for testing
        with torch.no_grad():
            for test_input, test_label in self.test_iter:
                test_input, test_label = test_input.to(self.devices), test_label.to(self.devices)
                _, _, tx_o = test_input[:, :3], test_input[:, 3:6], test_input[:, 6:9]

                # print("Tx_o shape: ", tx_o.shape)
                # print("tx_o: ", tx_o)
                gt_spectrum = test_label.detach().cpu()

                pred_spectrum = knn.predict(tx_o.detach().cpu().numpy())
                ...
                pred_spectrum = pred_spectrum.reshape(90, 360)
                gt_spectrum = gt_spectrum.reshape(90, 360)    
                pred_spectrum = pred_spectrum.numpy()
                gt_spectrum = gt_spectrum.numpy()            

                # print("num_spectrum: ", num_spectrum)
                # print("Test shape: ", gt_spectrum.shape)
                # print(gt_spectrum)
                # print("Pred shape: ", pred_spectrum.shape)
                # print(pred_spectrum)
                
                # pred2next = pred_spectrum[num_spectrum*360*90:]
                # gt2next = gt_spectrum[num_spectrum*360*90:]

                diff = np.abs(pred_spectrum - gt_spectrum)
                # print("Diff shape: ", diff.shape)
                # print(diff)
                pixel_error = np.mean(diff)
                ...
                self.logger.info("Median SSIM is {:.6f}\n".format(np.median(all_ssim)))
                # print("Median SSIM is {:.6f}\n".format(np.median(all_ssim)))
